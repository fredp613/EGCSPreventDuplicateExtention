...
namespace PreventDuplicateExtension.Plugins
{
    ...
    /// </summary>    
    public class PrePreventDuplicateFieldDelete : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
                ...
                context.InputParameters["Target"] is EntityReference)
            {
                ...
                if (entityRef.LogicalName != "fp_preventduplicatefield")
                    return;

                try
                {
                    var preventDuplicate = new PreventDuplicate(service, entityName);

                    if (!preventDuplicate.destroyPluginSteps())
                    {
                ...
                {
                    throw new InvalidPluginExecutionException("An error occurred in the prevent duplicate field plug-in.", ex);
                }

                catch (Exception ex)
                {
                    tracingService.Trace("prevent duplicate field Plugin: {0}", ex.ToString());
                    throw;
                }
